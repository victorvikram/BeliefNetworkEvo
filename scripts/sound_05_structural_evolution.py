"""
Sound 05: Structural Evolution — community stability and hub migration.

Usage: python scripts/sound_05_structural_evolution.py
Outputs: figures/sound_05_*.png, stdout
"""
import sys
...
def main():
    ...
    # Rebuild with fixed_vars for cross-window comparability
    print("\nRebuilding with fixed variables...")
    total_windows = build_rolling_windows(
        cleaned_df,
        window_size=4, step_size=2, min_years_per_window=3,
        min_n_per_group=100,
    ...
    print(f"Total windows (fixed): {len(total_windows)}")
    print(f"Group windows (fixed): {len(group_windows)}")

    # ══════════════════════════════════════════════════════════
    # Section A: Community Stability
    # ══════════════════════════════════════════════════════════
    print("\n" + "=" * 60)
    print("SECTION A: COMMUNITY STABILITY")
    print("=" * 60)
    ...
    ax.set_ylim(0, 1.05)
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "sound_05_nmi_trajectory.png", dpi=150, bbox_inches="tight")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "sound_05_community_switchers.png", dpi=150, bbox_inches="tight")
    plt.close()

    print(f"\n=== Top 20 Community Switchers (total network) ===")
    ...
    print(f"\n=== Stable Community Cores (>= 80% consistency) ===")
    print(f"Variables in stable cores: {len(stable_cores)} / {len(fixed_vars)}")
    ...
    # Show only fixed_vars, color by community label
    n_windows_total = len(total_comm_data)
    heatmap_vars = sorted(fixed_vars)
    ...
    ax.set_title("Community Membership Over Time (Total Network)")
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "sound_05_community_cores.png", dpi=150, bbox_inches="tight")
    plt.close()
    ...
    print("\n=== Centrality Rank Stability (consecutive window Spearman rho) ===")
    ...
    ax.set_ylim(0, 1.05)
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "sound_05_centrality_rank_corr.png", dpi=150, bbox_inches="tight")
    plt.close()
    ...
    ax.set_title("Top-10 Hub Variable Degree Centrality Over Time (Total Network)")
    ax.legend(fontsize=8, ncol=2)
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "sound_05_hub_degree_top10.png", dpi=150, bbox_inches="tight")
    plt.close()

    print(f"\n=== Top-10 Hub Variables (by mean degree centrality) ===")
    for v in top_hubs:
    ...
    ax.invert_yaxis()
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "sound_05_centrality_gainers_losers.png", dpi=150, bbox_inches="tight")
    plt.close()
    ...
    print(f"Stable core variables (>= 80%): {len(stable_cores)}/{len(fixed_vars)}")
    ...
    print("\nDone. Figures saved to figures/sound_05_*.png")
